...
def compute_distortion(url, cs):
    data_table = load_data_table(url)
    clist = []
    for line in data_table:
        clist.append(alg_cluster.Cluster(set([line[0]]), line[1], line[2], line[3], line[4]))

    cluster_list = alg_project3_solution.hierarchical_clustering(clist, cs)
    #cluster_list = alg_project3_solution.kmeans_clustering(clist, cs, 5)
    errlist = []
    for idx in cluster_list:
        errlist.append(idx.cluster_error(data_table))
    return int(sum(errlist))
